...
void saCalcSource(double ni, double ni_L, double S, double d, double rho, double drx, double dry, double dnix, double dniy, double* Qt)
{
    ...
    double Cv1 = 7.1;
    double X = ni/ni_L;
    double fv1 = X*X*X/(X*X*X + Cv1*Cv1*Cv1);

    double Cv2 = 5.;
    double k = 0.41;
    aux = 1./(1. + X/Cv2);
    double fv2 = aux*aux*aux;
    double fv3 = (1. + X*fv1)*(1. - fv2)/fmax(X, 0.001);
    double Stil = fv3*S + ni*fv2/(k*k*d*d);
