...
void boundaryCalcVisc(SOLVER* solver, MESHBC* bc)
{
    ...
    for(int ii=0; ii<bc->Nelem; ii++)
    {
        ...
        else
        {
	        ...
	        solver->eL[e0]->R[1] -= txx*dSx + txy*dSy;
	        solver->eL[e0]->R[2] -= txy*dSx + tyy*dSy;
	        solver->eL[e0]->R[3] -= u*(txx*dSx + txy*dSy) + v*(txy*dSx + tyy*dSy) + k*(dTx*dSx + dTy*dSy);            
        }
	}        
}
