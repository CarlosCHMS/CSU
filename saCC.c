...
void saCC_InterFace(SOLVER* solver)
{
    ...
    for(int ii=0; ii<solver->mesh->Ncon; ii++)
    {
        ...
        double drx, dux, dvx, dTx, dnx;
        double dry, duy, dvy, dTy, dny;

        if(solver->viscBlazek)
        {
            double drxm = (solver->dPx[0][e0] + solver->dPx[0][e1])*0.5;
            ...
            double drym = (solver->dPy[0][e0] + solver->dPy[0][e1])*0.5;
...
{
    int e0, p0, p1;
    double drx, dry, dux, duy, dvx, dvy, dnx, dny;
    ...
    dnx = dnxm - (dnxm*nx + dnym*ny)*nx;
    dny = dnym - (dnxm*nx + dnym*ny)*ny;
        ...
        if(strcmp(bc->name, solver->writeSurf) == 0)
        {
            ...
            {                                
                for(int kk=0; kk<Nvar; kk++)
                {
                    ...
                    solver->boundaryL[jj]->viscousFlux(solver->boundaryL[jj], solver, ii, f, &mi);
